...
use DBModule;
   ...
   our @EXPORT    = qw (
                        ...
                        get_user_f_and_l_name_by_name_from_db 
...
sub get_user_f_and_l_name_by_name_from_db {
   my ($uname) = @_;
    my $sql = "select FirstName, LastName  from user_info where user_name=\'$uname\';";
    my $qh = $db_exec->prepare ($sql) or die (@errors, '<br>' . "* $sql *" . $db_exec->errstr. '<br>' );
    $qh->execute()or die (@errors, '<br>' . "* $sql *" . $qh->errstr. '<br>' );
...
sub get_user_passwd_by_name_from_db{
    my ($uname) = @_;
    my $sql = "select user_passwd  from user_info where user_name=\'$uname\';";
    my $qh = $db_exec->prepare ($sql) or die (@errors, '<br>' . "* $sql *" . $db_exec->errstr. '<br>' );
    $qh->execute()or die (@errors, '<br>' . "* $sql *" . $qh->errstr. '<br>' );
